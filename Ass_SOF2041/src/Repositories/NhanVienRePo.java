...
public class NhanVienRePo implements INhanVienRepo {
    ...
    @Override
    public NhanVien update(NhanVien nv, String Ma) {
        try {
            Connection conn = getConnection();
            String query = " UPDATE NhanVien "
                    ...
                    + " WHERE Ma = ? ";
            PreparedStatement pr = conn.prepareStatement(query);
            pr.setString(1, nv.getTen());
            pr.setString(2, nv.getTenDem());
            pr.setString(3, nv.getHo());
            pr.setString(4, nv.getGioiTinh());
            pr.setDate(5, nv.getNgaySinh());
            pr.setString(6, nv.getDiaChi());
            pr.setString(7, nv.getSdt());
            pr.setString(8, nv.getMatKhau());
            pr.setString(9, nv.getIdCH());
            pr.setString(10, nv.getIdCV());
            pr.setInt(11, nv.getTrangThai());
            pr.setString(12, Ma);
            pr.execute();
        } catch (Exception ex) {
            Logger.getLogger(NhanVienRePo.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public NhanVien search(String ma) {
        ...
        try {
            ...
            while (ee.next()) {
                ...
                String idGBC = ee.getString("IdGuiBC");
                int trangthai = ee.getInt("TRANGTHAI");
                NhanVien nv1 = new NhanVien(id, ma1, ten, tendem, ho, gioitinh, ngaysinh, diachi, sdt, mk, idCH, idCV, idGBC, trangthai);
                nv = nv1;
            }
