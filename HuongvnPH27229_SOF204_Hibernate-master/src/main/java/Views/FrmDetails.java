...
public class FrmDetails extends javax.swing.JFrame {
    ...
    private DefaultComboBoxModel<ChucVu> comBoChucVu;
    ...
    private void addCbSanPham() {
        ...
        for (SanPham x : iSanPhamService.getAll()) {
    ...
    private void addSP() {
        ...
        if (iSanPhamService.getObjByMa(txt_ttsp_ma.getText()) == null) {
    ...
    private void updateSP() {
        ...
        if (row == -1) {
            ...
            SanPham sp = iSanPhamService.getObjByMa((String) tb_qlsp_ttsp.getValueAt(row, 1));
            if (checkNullSP()) {
                return;
            }
            sp.setTen(txt_ttsp_ten.getText());
    ...
    private void deleteSP() {
        ...
        if (row == -1) {
            ...
            if (helper.confirm(this, "Xác nhận muốn xóa ?")) {
                if (iSanPhamService.delete(sp).equals("NotOK")) {
    ...
    private void btn_cv_themActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_cv_themActionPerformed
        ...
        if (iChucVuService.getObjByMa(txt_cv_ma.getText()) == null) {
            cv.setMa(txt_cv_ma.getText());
            cv.setTen(txt_cv_ten.getText());
            iChucVuService.save(cv);
            loadData();
            helper.alert(this, "Thêm thành công!");
        } else {
    ...
    private void btn_cv_capnhatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_cv_capnhatActionPerformed
        ...
        if (row == -1) {
            ...
            ChucVu cv = iChucVuService.getObjByMa((String) tb_qlnv_chucvu.getValueAt(row, 1));
    ...
    private void btn_cv_xoaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_cv_xoaActionPerformed
        ...
        if (row == -1) {
            ...
            if (helper.confirm(this, "Xác nhận muốn xóa ?")) {
                ChucVu cv = iChucVuService.getObjByMa((String) tb_qlnv_chucvu.getValueAt(row, 1));
                if (iChucVuService.delete(cv).equals("NotOK")) {
                    helper.error(this, "Xóa thất bại!");
                } else {
                    loadData();
                    clearCV();
