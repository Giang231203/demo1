...
import Repo.ChiTietSanPhamRepo;
import Repo.IChiTietSanPhamRepo;
...
public class ChiTietSPService implements IChiTietSanPhamService{
    ...
    @Override
    public ArrayList<QLChiTietSanPham> timkiem(int NamBH) {
        ArrayList<QLChiTietSanPham> list = new ArrayList<>();
        var x = ctspRP.findata(NamBH);
        for (ChiTietSanPham c : x) {
            list.add(new QLChiTietSanPham(c.getiD(),c.getIdSP(),c.getIdNSX(),c.getIDMauSac(),c.getIdDongSP(),c.getNamBH(),c.getMoTa(),c.getSoLuongTon(),c.getGiaNhap(),c.getGiaBan()));
        }
        return list;
    }
}
